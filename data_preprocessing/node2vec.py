...
class node2vec:
    ...
    #  Code implementation of figure2 in the paper: get the node-to-node probability (for cases that are not second sampling)
    ...
    def alias_setup(self, probs):
        """Alias Sample strategy：The purpose of the alias_setup is to generate two numbers per node based on the probability of the second-order random walk output,
        which will be sampled by the alias_draw function later on.
        :probs: Probability of v to all x
        :return: Alias and Prob
        """
        K = len(probs)
        q = np.zeros(K)                  # Corresponds to the Prob array
        J = np.zeros(K, dtype=np.int)    # Corresponds to the Alias array
    ...
    def get_walks(self):
        ...
        g = self.G
        nodes = list(g.nodes())
        # print("unshuffled graph nodes:",nodes)
        for t in range(self.args.r):                     # Each node generates arg.r walk sequences
            np.random.shuffle(nodes)
            for node in nodes:
                walk = self.node2vecWalk(node)           # Generate 1 walk sequence per node
                walks.append(walk)
        # embedding
        #print('shuffled graph nodes:', nodes)             # The nodes are different each time when the graph is generated
        walks = [list(map(str, walk)) for walk in walks]  # The walks store each node's arg.r entry in a random walk path of length arg.l.
